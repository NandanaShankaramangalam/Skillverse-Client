...
function ForgotPasswords() {
    ...
    const handleSendOtp = async(e:React.MouseEvent<HTMLButtonElement, MouseEvent>) =>{
     e.preventDefault();
     const result = await api.post('/check-student',{email},{withCredentials:true})
      ...
      <div className="max-w-md items-center justify-center  border border-gray-300 rounded-md bg-white mx-4 sm:mx-auto ">
        ...
        <form action="" className=" w-full ps-6 pe-6 pt-6">
          <div className="mb-3 mt-5">
            ...
            />
            <button onClick={handleSendOtp} className="bg-custom-blue text-white ml-1 w-24 text-sm rounded-md hover:bg-gray-700 transition duration-150 ease-out">
                send otp
            </button>
            </div>
         ...
         <div>
          ...
          <div className={`flex justify-center pt-4`}>
            <button onClick={handleOtpSubmit} className="bg-custom-blue text-white py-2 px-6 text-sm rounded-md w-full hover:bg-gray-700 transition duration-150 ease-out" >
              Submit
            </button>
          </div>
