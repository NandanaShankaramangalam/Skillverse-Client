...
function RatingsAndReviews(props: ratingProps) {
  ...
  const handleRating = async(e:React.MouseEvent<HTMLButtonElement, MouseEvent>) =>{
    e.preventDefault();
   const result = await api.post('/rating',{selectedValue,studId,courseId},{withCredentials:true})
   if(result){
  ...
  return (
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 ">
      ...
      {/* <div className='flex justify-end'>
                <button onClick={props.setIsOpen(false)}><FontAwesomeIcon icon={faXmark}></FontAwesomeIcon></button>
        </div> */}
      {/* <h1>How do you rate this course?</h1> */}
      <div className="feedback absolute">
        <button onClick={(e)=>{handleRating(e);props.setIsOpen(false)}} className="absolute bottom-0 px-3 py-2 rounded shadow bg-blue-500 text-white  mb-5">Submit</button>
        <div className="flex w-full justify-end">
          {/* <div className='flex justify-end'>
          <button onClick={props.setIsOpen(false)}><FontAwesomeIcon icon={faXmark}></FontAwesomeIcon></button>
        </div> */}
          <button onClick={() => props.setIsOpen(false)}>
            <FontAwesomeIcon className="text-xl" icon={faXmark}></FontAwesomeIcon>
          </button>
        </div>
        <h1 className="text-xl font-bold">How do you rate the course?</h1>
        <div className="rating pb-5">
          <input type="radio" name="rating" value='5'
          checked={selectedValue === '5'}
          onChange={handleRadioChange} id="rating-5" />
          <label htmlFor="rating-5"></label>

          <input type="radio" name="rating" value='4'
          checked={selectedValue === '4'}
          onChange={handleRadioChange} id="rating-4" />
          <label htmlFor="rating-4"></label>

          <input type="radio" name="rating" value='3'
          checked={selectedValue === '3'}
          onChange={handleRadioChange} id="rating-3" />
          <label htmlFor="rating-3"></label>

          <input type="radio" name="rating" value='2'
          checked={selectedValue === '2'}
          onChange={handleRadioChange} id="rating-" />
          <label htmlFor="rating-2"></label>

          <input type="radio" name="rating" value='1'
          checked={selectedValue === '1'}
          onChange={handleRadioChange} id="rating-1" />
          <label htmlFor="rating-1"></label>

          <div className="emoji-wrapper pb-5">
            <div className="emoji pb-10">
