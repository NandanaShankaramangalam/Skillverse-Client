  ...
  return (
    <div className="mt-5">
      <div className="flex justify-end mr-11 mb-2">
        <button
          className="bg-yellow-500 rounded-lg text-white py-2 px-3 hover:bg-yellow-400"
          onClick={openModal}
        >
          Add{" "}
          <FontAwesomeIcon
            ...
            {data.map((item, index) => (
              ...
              >
                ...
                <td className="px-6 py-4">
                  <div className="pt-1">
                    <button
                      onClick={() => openAddModal(item._id, item.category)}
                    >
                      <FontAwesomeIcon
                        icon={faPlus}
                        className="text-white text-sm bg-cyan-700 p-1 mr-8"
                      ></FontAwesomeIcon>
                    </button>
                    <button
                      onClick={() => openEditModal(item._id, item.category)}
                    >
                      <FontAwesomeIcon
                        icon={faPenToSquare}
                        className="text-white text-sm bg-gray-700 p-1"
                      />
                    </button>
                  </div>
                </td>
                <td className="px-6 py-4 text-right">
                  {item.status ? (
                    <button
                      className="rounded-full border px-7 py-2 border-green-600 bg-green-600 text-white"
                      onClick={(e) => {
                        handleList(e, item._id);
                        notifyCatInactive();
                      }}
                    >
                      Active{" "}
                    ...
                    <button
                      className="rounded-full border px-6 py-2 border-red-600 bg-red-600 text-white"
                      onClick={(e) => {
                        handleUnlist(e, item._id);
                        notifyCatActive();
                      }}
                    >
                      Inactive
